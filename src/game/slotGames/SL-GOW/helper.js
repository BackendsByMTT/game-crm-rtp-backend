...
function checkForfeatureAll(inst) {
    try {
        const { settings } = inst;
        let foundSymbol = -1;
        for (let rowIndex = 0; rowIndex < settings.resultSymbolMatrix.length; rowIndex++) {
            const row = settings.resultSymbolMatrix[rowIndex];
            for (let colIndex = 0; colIndex < row.length; colIndex++) {
                const symbol = row[colIndex];
                if (symbol == settings.blueWild.SymbolID ||
        ...
        return {
            found: true,
            sym: foundSymbol === -1 ? settings.featureAllMult.length - 1 : foundSymbol,
        };
    }
    catch (e) {
...
function calculateFeaturePayout(inst, mult) {
    try {
...
function checkForWin(gameInstance) {
    try {
        ...
        if (found) {
            ...
            const pay = calculateFeaturePayout(gameInstance, settings.featureAllMult[sym]);
            totalPayout = pay;
        }
