...
const updateStatus = (client, status) => __awaiter(void 0, void 0, void 0, function* () {
...
const updateCredits = (client, creator, credits) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (client.role === "player") {
            const playerData = yield redis_1.redisClient.pubClient.hGetAll(`playground:${client.username}`);
            if (playerData && playerData.currentGame && playerData.currentGame !== "none") {
                throw (0, http_errors_1.default)(409, `Cannot update credits while the player is in a game: ${playerData.currentGameId}`);
            }
        }
        ...
        if (client.role === "player") {
            yield redis_1.redisClient.pubClient.publish(`player:${client.username}`, JSON.stringify({
                type: "UPDATE_BALANCE",
                payload: { credits: client.credits },
            }));
        }
        else {
            yield redis_1.redisClient.pubClient.publish(`control:${client.role}:${client.username}`, JSON.stringify({
                type: "UPDATE_BALANCE",
                payload: { credits: client.credits },
            }));
        }
        yield redis_1.redisClient.pubClient.publish(`control:${creator.role}:${creator.username}`, JSON.stringify({
            type: "UPDATE_BALANCE",
