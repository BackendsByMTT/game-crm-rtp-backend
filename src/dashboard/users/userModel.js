...
exports.PlayerSchema.statics.getHierarchyUsers = function (username) {
    return __awaiter(this, void 0, void 0, function* () {
        const hierarchy = [];
        const player = yield this.findOne({ username: username }).populate("createdBy");
        if (!player || !player.createdBy) {
            console.error("Player not found or has no creator");
            return hierarchy;
        }
        let currentManager = yield User.findById(player.createdBy);
        while (currentManager && currentManager.createdBy) {
            hierarchy.push(currentManager);
            currentManager = yield User.findById(currentManager.createdBy);
        }
        return hierarchy;
    });
};
