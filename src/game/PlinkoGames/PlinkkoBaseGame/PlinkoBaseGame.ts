...
import { PlinkoBaseSettings } from "./types";

/**
 * Class representing the base game logic for Keno.
 * Implements the RequiredSocketMethods interface.
 */
export default class PlinkoBaseGame implements RequiredSocketMethods {
  public settings: PlinkoBaseSettings;
  playerData = {
    haveWon: 0,
    currentWinning: 0,
    totalBet: 0,
  ...
  private prepareSpin(data: any) {
    ...
    this.settings.selectedRows= this.settings.rows[data.rows];
    this.settings.selectedRisk = this.settings.risk[data.risk];
  }
