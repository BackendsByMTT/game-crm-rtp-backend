    ...
    getGameBySlug(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!slug) {
                    throw (0, http_errors_1.default)(400, "Game ID parameter is required");
                }
                // Check if player exists and is active
                const currentPlayer = yield userModel_1.Player.findOne({
                    username,
                    role,
                    status: "active"
                }).lean();
                ...
                const playerSession = yield sessionManager_1.sessionManager.getPlaygroundUser(username);
                if (!playerSession) {
                    console.log(`❌ No active session found for player ${username}`);
                    throw (0, http_errors_1.default)(403, "No active session found. Please reconnect to the platform");
                }
                ...
                if (config_1.config.env === 'development') {
                    ...
                    if (hostPattern.test(game.url)) {
                        ...
                        throw (0, http_errors_1.default)(401, "Unauthorized game provider");
                    }
                }
